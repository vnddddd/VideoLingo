...
def cmd_download_audio(args: argparse.Namespace) -> None:
    """Download only the source audio and prepare output/audio/raw.mp3."""
    from core._1_ytdlp import download_audio_ytdlp

    if _checkpoint_complete([RAW_AUDIO_FILE]):
        print(f"[SKIP] Download audio: {_display_path(RAW_AUDIO_FILE)} already exists.")
        return

    raw_audio = download_audio_ytdlp(args.url, cookies_path=args.cookies_path)
    _require_files([raw_audio], "download-audio output")
    print(f"\n[OK] Downloaded audio: {_display_path(raw_audio)}")
...
def _require_local_inputs() -> None:
    try:
        ...
        video = None
        print(f"[INFO] No source video resolved for local audio pipeline: {exc}")
    else:
        print(f"Source video: {_display_path(video)}")

    from core.utils.config_utils import load_key

    required_inputs: list[Path] = [RAW_AUDIO_FILE]
    demucs_enabled = bool(load_key("demucs"))
    if demucs_enabled:
        print("[INFO] config demucs=true; vocal/background will be reused if present or generated by the ASR/reference steps.")
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    download_audio = subparsers.add_parser(
        "download-audio",
        help="Download only source audio from a URL and create output/audio/raw.mp3.",
    )
    download_audio.add_argument("url", help="Video URL supported by yt-dlp.")
    download_audio.add_argument("--cookies-path", help="Optional Netscape-format cookie file for yt-dlp.")
    download_audio.set_defaults(func=cmd_download_audio)
